...
const teamMembers = [
  { name: "Alif Fata Fadhlillah", role: "UI/UX Designer, Frontend Developer", image: alifImage },
...
export default function OurTeamSection() {
  ...
  return (
    <section className="relative min-h-screen h-auto sm:h-screen w-full snap-start snap-always bg-gradient-to-br from-white via-gray-50 to-white flex items-center justify-center overflow-hidden py-16 sm:py-0">
        ...
        <div className="absolute flex animate-wave-loop" style={{ width: '300%' }}>
          {[...Array(4)].map((_, i) => (
            <svg
              ...
              className="h-48 sm:h-64 opacity-25 text-[#5F7F7A] flex-shrink-0"
              style={{ width: '100%' }}
            >
              <path
        ...
        <div className="absolute flex animate-wave-loop-slow" style={{ width: '300%', animationDelay: '-2s' }}>
          {[...Array(4)].map((_, i) => (
            <svg
              ...
              fill="none"
              preserveAspectRatio="none"
              className="h-56 sm:h-72 opacity-18 text-[#CED7B0] flex-shrink-0"
              style={{ width: '100%' }}
            >
              <path
        ...
        <div className="absolute flex animate-wave-loop-slower" style={{ width: '300%', animationDelay: '-4s' }}>
          {[...Array(4)].map((_, i) => (
            <svg
              ...
              fill="none"
              preserveAspectRatio="none"
              className="h-64 sm:h-80 opacity-12 text-[#E8E5D5] flex-shrink-0"
              style={{ width: '100%' }}
            >
              <path
      ...
      <div className="relative z-10 w-full h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-center">
        <div className="relative w-full h-full sm:max-h-[85vh] flex flex-col lg:flex-row items-center justify-center">
          
          {/* Center Content with Animated Background */}
          <div className="relative z-20 text-center max-w-2xl px-4 sm:px-6 mb-8 sm:mb-12 lg:mb-0 w-full lg:w-auto">
            {/* Decorative Elements Behind Text */}
            <div className="absolute inset-0 -z-10">
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-48 sm:w-64 sm:h-64 bg-[#5F7F7A]/10 rounded-full blur-3xl animate-pulse-ring"></div>
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-36 h-36 sm:w-48 sm:h-48 bg-[#CED7B0]/20 rounded-full blur-2xl animate-pulse-ring" style={{ animationDelay: '1s' }}></div>
            </div>

            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-gray-900 mb-3 sm:mb-4 leading-tight relative">
              <span className="text-3xl sm:text-4xl font-medium text-[#1C1C1C] tracking-tight">
                The Minds Behind{' '}
                <span className="font-bold text-[#4C6F68]">SentiLoka</span>
          ...
          {/* Team Cards - Desktop Only (unchanged) */}
          {teamMembers.map((member, index) => (
            <div
              key={index}
              className={`absolute hidden lg:block transition-all duration-500 ease-out ${positions[index]}`}
              style={{ 
          ...
          {/* Mobile & Tablet View - Grid Below Title */}
          <div className="lg:hidden w-full max-w-2xl mx-auto px-4">
              ...
              {teamMembers.map((member, index) => (
                ...
                >
                  ...
                  <div className="relative w-full max-w-[140px] sm:max-w-[160px] mx-auto">
                    ...
                    <div className="absolute -inset-1 bg-gradient-to-r from-[#5F7F7A] to-[#CED7B0] rounded-2xl opacity-0 group-active:opacity-50 blur transition-opacity duration-300"></div>
                    
                    <img
                      src={member.image}
                      alt={member.name}
                      className="relative w-full aspect-square rounded-2xl object-cover shadow-lg transition-all duration-300 group-active:scale-95 border-2 border-white"
                      onError={(e) => {
                        e.target.src = `https://via.placeholder.com/150x150/cccccc/000000?text=${member.name
                          .split(' ')[0]
                          .charAt(0)}`;
                  ...
                  {/* Name & Role */}
                  <div className="text-center mt-2 sm:mt-3 w-full px-1 max-w-[140px] sm:max-w-[160px] mx-auto">
                    <h4 className="text-[11px] sm:text-xs font-bold text-gray-900 mb-0.5 sm:mb-1 leading-tight">
                      {member.name}
                    </h4>
                    <p className="text-[9px] sm:text-[10px] text-gray-600 leading-snug">
                      {member.role}
                    </p>
                  </div>
                </div>
